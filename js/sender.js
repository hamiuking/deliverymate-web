...
function enforceSenderGate() {
  ...
  if (status) {
    if (locked) {
      status.textContent =
        "Pilot: please register first to enable request creation and offer acceptance on this device.";
    } else {
      let phone = "";
      try {
        phone =
...
function setupCreateRequest() {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    if (!data.sender_ack_version) {
      data.sender_ack_version =
        sessionStorage.getItem("dm_sender_ack_version") || "v2";
    }
    ...
    try {
      ...
      if (u && u.full_name)
        data.sender_name = data.sender_name || u.full_name;
    } catch (_) {}
    ...
    if (res.ok) {
      ...
      setResult(
        result,
        alertSuccess(
          `You have created Request ID: ${res.request?.id ?? "unknown"}`
        )
      );
    } else {
      setResult(result, alertError(res.error || "Failed"));
...
function renderCreateRequestInfo(res) {
  try {
    ...
    if (viewForm && viewForm.request_id)
      viewForm.request_id.value = String(id);
    ...
    box.innerHTML = `
      <div class="card compact" style="border:1px solid rgba(15,23,42,.12);">
        <div><strong>You have created Request ID:</strong> <span id="createdRequestId">${safeText(
          id
        )}</span></div>
        <div class="btn-row" style="margin-top:8px;">
          <button class="btn secondary" type="button" id="copyRequestIdBtn">Copy ID</button>
          <button class="btn" type="button" id="loadCreatedRequestBtn">View this request</button>
        </div>
...
function setupViewRequest() {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    if (req.ok) {
      if (req.request) addSenderRecent(req.request);
...
function renderSenderSummary({
  ...
  if (!req || !req.ok || !req.request) {
    summary.insertAdjacentHTML(
      "beforeend",
      alertError(req?.error || "Failed to load request")
    );
    return;
  }
  ...
  summary.insertAdjacentHTML(
    ...
    <div class="card compact">
      ...
      <div class="muted" style="margin-top:10px;">
        Request #${r.id} · ${safeText(r.pickup_suburb)} → ${safeText(
      r.dropoff_suburb
    )}
      </div>
    </div>
  ...
  const canConfirm =
    r.status === "delivered" || r.escrow_status === "pending_release";
  if (canConfirm) {
    summary.insertAdjacentHTML(
      "beforeend",
      `
  ...
  const arr =
    offers && offers.ok && Array.isArray(offers.offers) ? offers.offers : [];
  if (offers && !offers.ok) {
    offersList.insertAdjacentHTML(
      "beforeend",
      alertError(offers.error || "Failed to load offers")
    );
  } else if (arr.length === 0) {
    offersList.insertAdjacentHTML(
      "beforeend",
      `<div class="muted">No offers yet.</div>`
    );
  } else {
    offersList.insertAdjacentHTML(
      "beforeend",
          ...
          <tbody>
            ${arr
              ...
              <tr>
                ...
                <td>${
                  o.price_nzd != null ? `$${safeText(o.price_nzd)}` : ""
                }</td>
  ...
  const h =
    hist && hist.ok && Array.isArray(hist.history) ? hist.history : [];
  if (hist && !hist.ok) {
...
function setupFundEscrow() {
  ...
  btn.addEventListener("click", async () => {
    ...
    if (res.ok && res.checkout_url) {
      ...
      setResult(
        result,
        alertError(res.error || "Failed to start payment")
      );
    }
  });
}
...
function setupSenderAuthControls() {
  ...
  if (continueBtn) {
    continueBtn.addEventListener("click", () => {
      ...
      if (!u) {
        if (hint)
          hint.textContent =
            "No saved login found. Please register below.";
        return;
      }
      localStorage.setItem("dm_sender_registered", "1");
  ...
  if (logoutBtn) {
    logoutBtn.addEventListener("click", () => {
      ...
      if (hint)
        hint.textContent =
          "Logged out. Please register again to use this device.";
    });
  }
}
...
function setupSenderLogin() {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    if (hint)
      hint.textContent = `Logged in as ${res.user?.phone || phone}`;
  });
}
